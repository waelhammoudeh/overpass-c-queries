/******** 		Function source : *************
 *		http://draft.scyphus.co.jp/lang/c/url_parser.htm
 *
 *	EDIT: removed all "http_" prefixes from url_parser.c file W.H.
 *	Added check for null on top of function. I do not allow NULL
 *	or empty string.
 *	Get our own copy to parse with strdup().
 * Changed prototype for function parsed_url_free() from taking
 * a pointer to "POINTER to POINTER" parameter, new prototype:
 * void parsed_url_free(struct parsed_url **);
 * And replaced ALL function calls accordingly. W.H.
 * All fields - except server (host) name, are optional. We should NOT
 * return NULL for missing any field other than the host.
 * This function is ALL wrong. Needs redoing. W.H. FIXME
 ******************************************************************/
...
parse_url(const char *url)
{
    ...
    // do not allow null W.H.
    if (url == NULL)  return NULL;

    // no empty string either
    ...
    // curstr = url; wrong! better get your own copy with strdup() W.H
    curstr = strdup(url);
    if (curstr == NULL)  return NULL; // malloc may fail!
 ...
 */
void
parsed_url_free(struct parsed_url **p2purl)
{
	struct parsed_url *purl;

	purl = (struct parsed_url *) *p2purl;
